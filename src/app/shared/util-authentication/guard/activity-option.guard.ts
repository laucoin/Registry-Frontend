...
export const activityOptionGuard: CanActivateFn = (): Promise<boolean> | boolean => {
    ...
    return currentUser?.preferences?.selectedProfile?.event?.options?.some( (option: SelectItem<string>): boolean => option.value === 'ACTIVITY' ) ?? false
}
