...
/**
 * The authenticated global home: a personal launchpad. Four panels,
 ...
 */
const asProfilePage = (data: unknown): PageDto<ProjectProfileRowDto> | undefined =>
		data as PageDto<ProjectProfileRowDto> | undefined
const asOpenAlerts = (data: unknown): OpenAlertProjectDto[] | undefined => data as OpenAlertProjectDto[] | undefined
		...
		<div class="dashboard__grid">
			...
			>
				<template #default="{ data }: { data: unknown }">
					<DashboardEmptyHint
							v-if="!asProfilePage(data)?.content?.length"
					...
					>
						<li
								v-for="profile in asProfilePage(data)?.content"
								:key="profile.id"
								class="dash-row"
						>
							<NuxtLink
			...
			>
				<template #default="{ data }: { data: unknown }">
					...
					>
						<li
								v-for="project in asOpenAlerts(data)"
								:key="project.id"
								class="dash-row"
						>
							<NuxtLink
