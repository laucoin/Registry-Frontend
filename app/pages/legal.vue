...
// The hosting provider is deploy-specific (ADR 023), so it comes from runtime
...
const hosting = computed(() => ({
	name: legalHosting.name?.trim() ?? '',
	address: legalHosting.address?.trim() ?? '',
	phone: legalHosting.phone?.trim() ?? '',
}))
