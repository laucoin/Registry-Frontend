...
import type { AssignableParticipantDto, GroupRowDto, PageDto, ParticipantRowDto } from '@shared/utils/api-types'
import { useSessionStore } from '@stores/session'
import { Alert, Button, Drawer, Empty, Input, List, ListItem, Modal, Space, Tag } from 'ant-design-vue'
...
// Managing membership is a group update.
...
async function openEdit(g: GroupRowDto): Promise<void> {
	...
	try {
		...
		preservedMembers.value = full.members?.map(m => m.id) ?? []
...
function confirmDelete(group: GroupRowDto): void {
	Modal.confirm({
		...
		okType: 'danger',
		okButtonProps: { 'data-testid': 'group-delete-confirm' },
		cancelText: t('common.cancel'),
		onOk: () => write.remove(group.id),
	})
}

// Members management — the §4 eligibility-picker reference.
const membersDrawerOpen = ref(false)
const activeGroup = ref<GroupRowDto | null>(null)
const members = ref<ParticipantRowDto[]>([])
...
async function loadMembers(groupId: string): Promise<void> {
	...
	try {
		const page = await $fetch<PageDto<ParticipantRowDto>>(`${basePath()}/${groupId}/members`, {
...
async function addMembers(): Promise<void> {
	...
	try {
		...
		membersError.value = apiErrorMessage(error)
	}
}
