...
// ADR 014 (as amended) — global store: auth/current session, consumed directly
// by components. Auth orchestration (ADR 022) lives in the actions: components
...
export const useSessionStore = defineStore('session', {
	...
	getters: {
		...
		// Project-scoped authority (ADR 005): the namespaced `{projectId}_PERM`
	...
	actions: {
		...
		// Full-page redirect into the BFF login route (ADR 022) — not an
		// in-app navigation. Callers pass the path to return to.
		login(redirectTo = '/'): void {
			navigateTo(`/auth/login?redirect=${encodeURIComponent(redirectTo)}`, { external: true })
		},
