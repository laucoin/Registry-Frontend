...
// ADR 020 — RUM + error capture behind the BFF /telemetry seam. PII
...
export default defineNuxtPlugin((nuxtApp) => {
	...
	function captureError(error: unknown, source: ErrorEntry['source']): void {
		// The server schema rejects oversized batches wholesale, so an error storm
		// (render loop, repeated rejections) must not grow the buffer past the
		// contract cap — the first errors are the diagnostic ones anyway.
