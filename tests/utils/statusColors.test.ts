...
describe('STATUS_COLOR', () => {
	it('exposes exactly the five status intents', () => {
		// Arrange — the palette module

		// Act
		const keys = Object.keys(STATUS_COLOR).sort()

		// Assert
		expect(keys).toEqual(['accent', 'danger', 'info', 'neutral', 'success'])
	})

	it.each(Object.entries(STATUS_COLOR))('%s is a six-digit hex colour', (_intent, hex) => {
		// Arrange — palette entry
	...
	it.each(Object.entries(STATUS_COLOR))('%s clears WCAG AA (≥ 4.5:1) with white text', (_intent, hex) => {
		...
		// Assert — ADR 015: solid dark backgrounds keep white text AA-safe
		expect(ratio).toBeGreaterThanOrEqual(4.5)
	})

	it('fails the AA check for a colour the palette must never contain (guard on the helper)', () => {
		// Arrange — AntD's tinted-tag green text tone, the very thing this
		// palette replaces
		...
		// Assert — the helper genuinely discriminates
