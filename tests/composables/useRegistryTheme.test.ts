...
describe('useRegistryTheme', () => {
	...
	it.each([
		['the configured dark canvas', { colorBgLayout: '#0f1013' }, '#0f1013'],
		['AntD\'s own dark canvas when the deploy overrides none', {}, '#000000'],
	])('inks solid buttons with %s, where white is unreadable', (_label, darkBlock, ink) => {
		// Arrange
		stubConfig({ colorPrimary: '#003a5d', dark: { colorPrimary: '#6ddcff', ...darkBlock } })
		usePreferencesStore().setThemeMode('DARK')
		...
		// Assert
		expect(themeConfig.value.components?.Button).toEqual({ colorTextLightSolid: ink })
	})

	it('leaves the light mode on AntD\'s white solid ink, which clears AA on the brand navy', () => {
		// Arrange
		stubConfig({ colorPrimary: '#003a5d', dark: { colorBgLayout: '#0f1013' } })

		// Act
		const themeConfig = useRegistryTheme()

		// Assert
		expect(themeConfig.value.components).toBeUndefined()
	})
