...
// ADR 022 — sealed-cookie session handling. The h3/Nitro helpers
...
// ADR 022 — the two-tier session lifetime: an absolute cap from login and a
...
// ADR 022 — __Host- prefix in production (https), plain names over local http.
...
describe('peekSession', () => {
	...
	it('returns null when the cookie fails to unseal (tampered → empty data, no user)', async () => {
		// h3 yields an empty data object when the sealed payload cannot be
		// verified, so a tampered cookie surfaces here as `user` being absent.
	...
	it('caps the touch throttle at half the idle window so short idle windows still refresh', async () => {
		freezeNow()
		// idleMaxAge 80 s → throttle is min(60 s, 40 s) = 40 s: 41 s-old activity
		// must re-seal even though it is under the 60 s default throttle.
