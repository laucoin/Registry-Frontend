...
/**
 * The OIDC client seam, exercised with openid-client fully mocked
...
describe('refreshAccessToken', () => {
	...
	/**
	 * The rotation race the replay window exists for: a request the browser sent
	 * before it received the rotated cookie still presents the CONSUMED token.
	 * Replaying the grant at the IdP is the reuse Authentik answers by revoking the
	 * whole family, which killed a live session and surfaced as a spurious
	 * re-login. The memoized result is handed out instead — consuming a given
	 * refresh token has exactly one outcome, so it can never be stale.
	 */
	it('replays the memoized result for a request that still carries the consumed token', async () => {
		stubNitroGlobals()
		vi.useFakeTimers()
		vi.setSystemTime(NOW)
		discovery.mockResolvedValue(FAKE_OIDC_CONFIG)
		refreshTokenGrant.mockResolvedValue({ access_token: 'at-2', refresh_token: 'rt-2', expires_in: 60 })
		const early = makeFakeSession({ refreshToken: 'rt-1' })
		const late = makeFakeSession({ refreshToken: 'rt-1' })
		const idp = await loadModule()

		await idp.refreshAccessToken(makeEvent(), asSession(early))
		await vi.advanceTimersByTimeAsync(1_000)
		await idp.refreshAccessToken(makeEvent(), asSession(late))

		expect(refreshTokenGrant).toHaveBeenCalledTimes(1)
		expect(late.data.accessToken).toBe('at-2')
		expect(late.data.refreshToken).toBe('rt-2')
		expect(late.clear).not.toHaveBeenCalled()
		vi.useRealTimers()
	})

	it('runs a fresh grant once the replay window has elapsed', async () => {
		stubNitroGlobals()
		vi.useFakeTimers()
		vi.setSystemTime(NOW)
		discovery.mockResolvedValue(FAKE_OIDC_CONFIG)
		...
		session.data.refreshToken = 'rt-1'
		await idp.refreshAccessToken(makeEvent(), asSession(session))

		expect(refreshTokenGrant).toHaveBeenCalledTimes(2)
		expect(refreshTokenGrant).toHaveBeenLastCalledWith(FAKE_OIDC_CONFIG, 'rt-1')
		vi.useRealTimers()
	})

	it('does not memoize a rejected grant, so a transient failure can be retried', async () => {
		stubNitroGlobals()
		discovery.mockResolvedValue(FAKE_OIDC_CONFIG)
		refreshTokenGrant
			.mockRejectedValueOnce(new TypeError('fetch failed'))
			.mockResolvedValueOnce({ access_token: 'at', refresh_token: 'rt-2', expires_in: 60 })
		const session = makeFakeSession({ refreshToken: 'rt-1' })
		const idp = await loadModule()

		await expect(idp.refreshAccessToken(makeEvent(), asSession(session))).rejects.toThrow('fetch failed')
		await expect(idp.refreshAccessToken(makeEvent(), asSession(session))).resolves.toBe(true)

		expect(refreshTokenGrant).toHaveBeenCalledTimes(2)
	})

	it('keeps the expiry the grant was issued with when its result is replayed', async () => {
		stubNitroGlobals()
		vi.useFakeTimers()
		vi.setSystemTime(NOW)
		discovery.mockResolvedValue(FAKE_OIDC_CONFIG)
		...
		expect(late.data.expiresAt).toBe(NOW + 60 * 1000)
		vi.useRealTimers()
	})
})
