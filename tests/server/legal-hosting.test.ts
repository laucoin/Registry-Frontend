...
/**
 * The hosting details on /legal are deploy-injected, so a production
...
function stubRuntimeConfig(production: boolean, legalHosting: Record<string, unknown>) {
...
describe('assertLegalHostingConfigured', () => {
	...
	/**
	 * Nuxt pipes NUXT_PUBLIC_* through destr, so a separator-free phone number
	 * reaches runtime config as a number. A naive .trim() would abort the boot
	 * with a TypeError naming nothing useful.
	 */
	it('accepts a phone number destr coerced to a number', async () => {
		stubRuntimeConfig(true, { ...COMPLETE, phone: 33972101007 })
		const { assertLegalHostingConfigured } = await loadModule()

		expect(() => assertLegalHostingConfigured()).not.toThrow()
	})

	it('reports a nullish field as missing rather than throwing', async () => {
		stubRuntimeConfig(true, { ...COMPLETE, address: undefined })
		const { assertLegalHostingConfigured } = await loadModule()
