...
export default defineNuxtConfig({
	...
	/**
	 * Global stylesheets. fonts.css first (the @font-face declarations the two
	 * sheets below reference), then responsive.css (layout floor + AntD portal
	...
	/**
	 * Runtime configuration. Everything here is a placeholder/default
	...
	runtimeConfig: {
		 ...
		 */
		registryBaseUrl: 'http://localhost:8081',
		production: false,
		appConfigPath: '',
		telemetry: {
			otlpEndpoint: '',
	...
	vite: {
		optimizeDeps: {
			 ...
			 * The nested "a > b" syntax names ant-design-vue's own resolution of
			 * dayjs, which is what has to be prebundled — our direct dependency on
			 * dayjs (for the picker adapters) is a separate graph.
			 */
			include: [
	...
	i18n: {
		...
		/**
		 * The html lang attribute is managed with the rest of the head.
		 */
